...
def run(p_seed=0, p_epochs=150, p_logdir="temp"):
    ...
    gen_optimizer = torch.optim.Adam(gen.parameters(), lr=5e-4)
    q_optimizer = torch.optim.Adam(params, lr=5e-4)
    ...
    for epoch in range(epochs):
        for idx, (data, target) in enumerate(data_loader):
            ...
            dis_real, _ = dis(data)
            dis_fake, cls_fake = dis(gen_images.detach())
            ...
            _, cls_fake = dis(gen_images.detach())
            q_loss = loss(Variable(cls_fake.detach(), requires_grad=True), torch.argmax(target,1))
            q_loss.backward()
            q_optimizer.step()
